...
def func(message):
    if (message.text == "Следующий спектакль"):
        if datetime([0][0]) == now.day - timedelta(days=1) and datetime(speki[0][2]) == now.month:
            bot.send_message(message.chat.id,text=f'Завта в {speki[0][0]} будет {speki[0][4]}\n Приходить к {speki[0][0]-timedelta(hours=1)}')
